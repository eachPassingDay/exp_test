...
class GaussianModel():
    ...
    def __init__(self, cfg):
        ...
        self._rotation = None
        self._opacity = None
        self._creation_frame_id = torch.empty(0) # <--- [NEW] Initialize creation_frame_id
        self._ghost_count = torch.empty(0)       # [NEW]
    ...
    #[NEW]
    def update_ghost_count(self, counts, indices=None):
        if indices is not None:
            self._ghost_count[indices] = counts
        else:
            self._ghost_count = counts
    # <--- [NEW] Getter for creation_frame_id
    def get_creation_frame_id(self):
        return self._creation_frame_id
    
    # <--- [NEW] Updater for creation_frame_id
    def update_creation_frame_id(self, ids, indices=None):
        if indices is not None:
            self._creation_frame_id[indices] = ids.detach().clone()
        else:
            self._creation_frame_id = ids.detach().clone()
    def get_scaling(self):
        return self._scaling
    ...
    def add_neural_points(self, batch_rays_o, batch_rays_d, batch_gt_depth, batch_gt_color,\
                           train=False, is_pts_grad=False, dynamic_radius=None, current_frame_id=0): # [NEW Arg]
        """
        ...
        if batch_rays_o.shape[0]:
            ...
            # <--- [NEW] Create frame ID tensor for new points
            new_ids = torch.full((pts.shape[0], 1), current_frame_id, dtype=torch.int, device="cuda")
            new_counts = torch.zeros((pts.shape[0], 1), dtype=torch.int, device="cuda")
            if self._xyz is None:
                ...
                self._creation_frame_id = new_ids # <--- [NEW]
                self._ghost_count = new_counts  #[NEW]
                ...
                self._creation_frame_id = torch.cat((self._creation_frame_id, new_ids), dim=0) # <--- [NEW]
                self._ghost_count = torch.cat((self._ghost_count, new_counts), dim=0) #[NEW]
    ...
    # =========================================================================
    # [手动添加] 2. 公开的剪枝接口 (给 Module 2 调用)
    # =========================================================================
    def prune_points(self, mask):
        valid_points_mask = ~mask
        
        # 1. 更新优化器 (最重要的一步，不更新会报错维度不匹配)
        ...
        # 4. 同步点云 (防止 FAISS 索引失效)
        self._xyz = self._xyz.detach() # 这里的 detach 很重要
        self._cloud_pos = self._xyz.tolist()
        self._pts_num = self._xyz.shape[0]